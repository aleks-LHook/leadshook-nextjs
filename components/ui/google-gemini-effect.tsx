...
export const GoogleGeminiEffect = ({
  ...
  const defaultLeftLabels = ["Attract", "Qualify", "Convert", "Track", "Optimize"];
  const defaultRightLabels = ["Page Builder", "Quiz Maker", "Server-Side Tracking", "SoC Compliance", "Performance"];
  ...
  return (
    <div className={cn("sticky top-80", className)}>
      ...
      >
        ...
        {/* Left side labels - Problems */}
        {leftText.map((label, idx) => (
          <motion.text
            key={`left-${idx}`}
            x="40"
            y={354 + idx * 75}
            ...
            fontWeight="700"
            className="font-sans"
            style={{ opacity: pathLengths[idx] }}
          >
            {label}
          </motion.text>
        ))}

        {/* Right side labels - Solutions */}
        {rightText.map((label, idx) => (
          <motion.text
            key={`right-${idx}`}
            x="1400"
            y={354 + idx * 75}
